...
const ResumeForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    reader.onload = async (e) => {
      ...
      emailjs
        .sendForm("service_x299nyh", "template_jknz8by", emailParams, "94TtzAZZ4eCvZYHzO")
        .then(
          (response) => {
  ...
  return (
    <form component="form" onSubmit={handleSubmit} sx={{ width: "100%", maxWidth: 800, marginTop: "20px", p: 3, backgroundColor: "#fff" }}>
      <Typography sx={{ textAlign: "center", fontSize: "25px", fontWeight: "bold", fontFamily: "circular", color: "#650909", mb: 2 }}>
      ...
      <Grid2 container spacing={4}>
        ...
        {/* Role Dropdown */}
        <Grid2 item sm={6} xs={12}>
          <Select fullWidth displayEmpty variant="standard" name="role" value={formData.role} onChange={handleChange}>
        ...
        {/* Position Dropdown */}
        <Grid2 item sm={6} xs={12}>
          <Select fullWidth displayEmpty variant="standard" name="position" value={formData.position} onChange={handleChange}>
        ...
        {/* Upload Resume */}
        <Grid2 item xs={12}>
          <input type="file" name={formData?.resume?.name} onChange={handleFileChange} accept=".pdf,.doc,.docx" />
        </Grid2>
        
        {/* Message */}
        <Grid2 item xs={12}>
          <TextField fullWidth name="message" value={formData.message} onChange={handleChange} placeholder="Project description" multiline rows={2.5} variant="standard" InputProps={{ startAdornment: (<InputAdornment position="start" style={{ marginBottom: "28px" }}><MessageIcon /></InputAdornment>) }} />
        </Grid2>
      </Grid2>
      
      {/* Submit Button */}
      <Box sx={{ display: "flex", justifyContent: "center", mt: 5, fontFamily: "circular" }}>
        <Button variant="contained" color="primary" sx={{ px: 3, py: 1, textTransform: "capitalize", backgroundColor: "#650909", "&:hover": { backgroundColor: "#fff", border: "1px solid #650909", color: "#650909" } }} type="submit">
          Send Enquiry
        </Button>
      </Box>
    </form>
